...
<html lang="{{ str_replace('_', '-', $locale) }}" dir="{{ $htmlDir }}">
    <head>
        ...
        <title inertia>{{ config('app.name', 'Learmy') }}</title>
        @php
            try {
        ...
        @else
            {{-- Fallback brand icon: PNG for modern browsers, .ico for legacy,
                 PNG apple-touch for iOS home-screen. --}}
        ...
        <script>
            window.fbAsyncInit = function() {
                FB.init({
                    ...
                    xfbml: false,
                    version: '{{ config("all.meta.api_version") }}',
                });
