...
const SETUP_GUIDES = {
    ...
    meta_app: {
        ...
        sections: [
            ...
            {
                heading: 'Step 3 — Embedded Signup (WhatsApp)',
                icon: 'signup',
                note: 'Lets your clients connect WhatsApp with a single button click instead of manual token entry.',
                steps: [
            ...
            {
                heading: 'Step 4 — Embedded Signup (Instagram / Messenger)',
                icon: 'signup',
                feature: 'facebook',
                note: 'Lets your clients connect Instagram & Messenger with a single button click.',
                steps: [
                    'In Facebook Login for Business → Configurations → Create Configuration.',
            ...
            {
                heading: 'Step 5 — Instagram Business DMs',
                icon: 'instagram',
                feature: 'instagram',
            ...
            {
                heading: 'Step 6 — Facebook Messenger DMs',
                icon: 'messenger',
                feature: 'facebook',
            ...
            {
                heading: 'Step 7 — Facebook & Instagram Social Posting',
                icon: 'social',
...
function SetupGuide({ provider }) {
    ...
    const rawGuide = SETUP_GUIDES[provider];
    if (!rawGuide) return null;

    let guide = { ...rawGuide };
    if (provider === 'meta_app') {
        const hasSocial = features.facebook !== false || features.instagram !== false || features.social_media !== false;
        if (!hasSocial) {
            guide.subtitle = 'Covers WhatsApp Business API · Embedded Signup';
        }
        if (Array.isArray(guide.sections)) {
            guide.sections = guide.sections
                .filter(sec => !sec.feature || features[sec.feature] !== false)
                .map((sec, idx) => ({
                    ...sec,
                    heading: sec.heading.replace(/^Step \d+/, `Step ${idx + 1}`)
                }));
        }
    }
