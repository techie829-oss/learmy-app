...
class IntegrationConfigController extends Controller
{
    public function index(): Response
    {
        ...
        foreach (IntegrationConfig::PROVIDERS as $provider) {
            ...
            // Dynamically hide categories based on config/all.php feature flags
            if ($category === 'Social OAuth' && !is_feature_enabled('social_media')) {
                continue;
            }
            if (($category === 'AI / LLM' || $category === 'Vector Store') && !is_feature_enabled('ai')) {
                continue;
            }
            ...
            if ($provider === 'meta_app') {
                $label = (is_feature_enabled('facebook') || is_feature_enabled('instagram'))
    ...
    public function edit(string $provider): Response
    {
        ...
        if ($provider === 'meta_app' && !is_feature_enabled('facebook') && !is_feature_enabled('instagram')) {
            $fields = array_values(array_filter($fields, fn ($f) => $f['key'] !== 'config_id_social'));
        }
    ...
    public function update(Request $request, string $provider): RedirectResponse
    {
        ...
        foreach ($fields as $f) {
            $rules['credentials.'.$f['key']] = [$f['required'] ? 'required' : 'nullable', 'string', 'max:1024'];
        }
