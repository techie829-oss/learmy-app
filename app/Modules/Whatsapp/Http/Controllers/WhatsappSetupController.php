...
class WhatsappSetupController extends Controller
{
    ...
    public function refreshPhoneStatus(Request $request, WhatsappBusinessAccount $waba, string $phoneNumberId): JsonResponse
    {
        ...
        $phoneExists = WhatsappPhoneNumber::where('waba_id_fk', $waba->id)->where('phone_number_id', $phoneNumberId)->exists();
        if (!$phoneExists) {
            abort(403, 'Phone number does not belong to this WABA.');
        }
