...
class MediaService
{
    ...
    {
        $gb = 5; // Default storage limit in GB when no subscription module is attached

        if (method_exists($user, 'effectiveSubscription')) {
            $plan = $user->effectiveSubscription()?->plan;
            if ($plan && method_exists($plan, 'limitValue')) {
                $gb = $plan->limitValue('storage_gb') ?? $gb;
            }
        }
